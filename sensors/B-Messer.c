...
void initIIS2MDC(void) {
    ...
    iis2mdc_operating_mode_set(&devMag, IIS2MDC_CONTINUOUS_MODE);
}
...
void getSensorDataMag(uint16_t *magX, uint16_t *magY, uint16_t *magZ) {
    uint8_t regData[6];
    iis2mdc_read_reg(&devMag,0x68,&regData[0],6);
    *magX = (regData[1] << 8) | regData[0];
    *magY = (regData[3] << 8) | regData[2];
    *magZ = (regData[5] << 8) | regData[4];
    *magX += 1000;
    *magY += 1000;
    *magZ += 1000;
}